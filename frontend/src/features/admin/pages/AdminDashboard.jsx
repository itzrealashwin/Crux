...
import { AlertCircle, ArrowUpRight, Briefcase as BriefcaseIcon, CheckCircle2, Download, FileCheck, Users } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const computed = useMemo(() => {
    const totalApplied = allJobs.reduce((acc, job) => acc + toNumber(job?.stats?.totalApplications), 0);
    const totalEligible = allJobs.reduce((acc, job) => acc + toNumber(job?.stats?.eligible), 0);
    const totalShortlisted = allJobs.reduce((acc, job) => acc + toNumber(job?.stats?.shortlisted), 0);
    const totalSelected = allJobs.reduce((acc, job) => acc + toNumber(job?.stats?.selected), 0);
    const totalHired = allJobs.reduce((acc, job) => acc + toNumber(job?.stats?.hired), 0);
    ...
    const drivesWithoutTimeline = openJobs.filter((job) => !job?.driveTimeline?.length).length;

    const studentsTotal = toNumber(summary.students?.value);
    const placementRate = toNumber(summary.placementRate?.value);
    const studentsPlaced = studentsTotal && placementRate
      ? Math.round((studentsTotal * placementRate) / 100)
      : totalHired;

    const packages = allJobs.map((job) => toNumber(job?.packageLPA, null)).filter((value) => value !== null);
    const avgPackage = packages.length
      ? (packages.reduce((acc, value) => acc + value, 0) / packages.length).toFixed(1)
      : null;
    ...
    const companiesVisited = new Set(allJobs.map((job) => job?.company?.name).filter(Boolean)).size;
  ...
  const needsAction = [
    ...
    },
    {
      ...
      count: toNumber(profileVerificationCounts.studentsWithIncompleteProfiles, 0),
      color: 'bg-blue-500/10 text-blue-500',
    ...
    <div className="min-h-screen bg-background text-foreground font-sans pb-20 space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">{todayLabel} • Live placement overview</p>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="outline" className="bg-transparent border-border text-foreground hover:bg-accent" onClick={() => navigate('/admin/jobs')}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <Card className="bg-card border-border shadow-sm rounded-xl">
          <CardContent className="p-5">
            ...
            <h3 className="text-3xl font-bold text-foreground mb-2">{computed.studentsPlaced}</h3>
            <p className="text-[10px] text-muted-foreground font-medium">
              {computed.placementRate.toFixed(1)}% of batch · <span className="text-emerald-500">{toNumber(summary.placementRate?.change, 0)}% MoM</span>
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-card border-border shadow-sm rounded-xl">
          <CardContent className="p-5">
            ...
            <h3 className="text-3xl font-bold text-foreground mb-2">{toNumber(summary.activeJobs?.value, openJobs.length)}</h3>
            <p className="text-[10px] text-muted-foreground font-medium">{computed.closingSoonCount} closing in 7 days</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-card border-amber-500/30 shadow-sm rounded-xl">
          <CardContent className="p-5">
            ...
            <h3 className="text-3xl font-bold text-amber-500 mb-2">{computed.pendingReviewCount}</h3>
            <p className="text-[10px] text-amber-500/80 font-medium">Derived from applied vs shortlisted</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-card border-border shadow-sm rounded-xl">
          <CardContent className="p-5">
            ...
            <h3 className="text-3xl font-bold text-foreground mb-2">{toNumber(summary.students?.value, 0)}</h3>
            <p className="text-[10px] text-muted-foreground font-medium">{toNumber(summary.students?.change, 0)}% vs last month</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-card border-border shadow-sm rounded-xl">
          <CardContent className="p-5">
            ...
            <h3 className="text-3xl font-bold text-red-500 mb-2">{computed.pendingOffers}</h3>
            <p className="text-[10px] text-muted-foreground font-medium">{computed.offerAcceptRate}% offer accept rate</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="bg-card border-border shadow-sm rounded-xl overflow-hidden">
        <CardContent className="p-0">
          <div className="grid grid-cols-3 md:grid-cols-6 divide-x divide-border text-center">
            <div className="p-5">
              <h4 className="text-2xl font-bold text-foreground">{computed.totalApplied}</h4>
            ...
            <div className="p-5">
              <h4 className="text-2xl font-bold text-foreground">{computed.totalEligible}</h4>
              <p className="text-xs text-muted-foreground font-medium mt-1">Eligible</p>
              <p className="text-[10px] text-emerald-500 mt-1">{getPercent(computed.totalEligible, computed.totalApplied)}% pass rate</p>
            </div>
            <div className="p-5">
              <h4 className="text-2xl font-bold text-foreground">{computed.totalShortlisted}</h4>
              <p className="text-xs text-muted-foreground font-medium mt-1">Shortlisted</p>
              <p className="text-[10px] text-muted-foreground mt-1">{getPercent(computed.totalShortlisted, computed.totalEligible)}% of eligible</p>
            </div>
            <div className="p-5">
              <h4 className="text-2xl font-bold text-foreground">{computed.inInterview}</h4>
              <p className="text-xs text-muted-foreground font-medium mt-1">In interview</p>
              <p className="text-[10px] text-muted-foreground mt-1">{getPercent(computed.inInterview, computed.totalShortlisted)}% of shortlisted</p>
            </div>
            <div className="p-5 bg-emerald-500/5">
              <h4 className="text-2xl font-bold text-foreground">{computed.totalSelected}</h4>
              <p className="text-xs text-muted-foreground font-medium mt-1">Selected</p>
              <p className="text-[10px] text-emerald-500 mt-1">{getPercent(computed.totalSelected, computed.totalShortlisted)}% conversion</p>
            </div>
            <div className="p-5 bg-emerald-500/5">
              <h4 className="text-2xl font-bold text-foreground">{computed.totalHired}</h4>
              <p className="text-xs text-muted-foreground font-medium mt-1">Hired</p>
              <p className="text-[10px] text-emerald-500 mt-1">{computed.offerAcceptRate}% accept rate</p>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <div className="lg:col-span-5 space-y-6">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {toNumber(job?.stats?.totalApplications)} applicants · Deadline {formatDate(job.deadline)}
                        </p>
                      </div>
                    </div>
                    <Badge variant="outline" className="font-normal text-[10px] px-2.5 py-0.5 rounded-full border-transparent whitespace-nowrap bg-emerald-500/10 text-emerald-500">
                      Open
                    </Badge>
                  </div>
                ))}
                {!openJobs.length && <p className="p-6 text-xs text-muted-foreground">No open drives available.</p>}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-card border-border shadow-sm rounded-xl">
            <CardHeader className="p-6 pb-2 flex flex-row items-center justify-between">
              ...
              <Button variant="outline" size="sm" className="h-8 bg-transparent border-border text-xs" onClick={() => navigate('/admin/jobs')}>
                Full analytics →
              </Button>
            </CardHeader>
            <CardContent className="p-6 space-y-5">
              {departmentPlacementRates.slice(0, 6).map((item) => {
                ...
                const barColor = rate >= 50 ? 'bg-emerald-500' : rate >= 30 ? 'bg-amber-500' : 'bg-red-500';

                return (
                  <div key={item.department}>
                    <div className="flex justify-between text-xs font-medium mb-2">
                      <span className="text-muted-foreground">{item.department}</span>
                      <span className="text-foreground">{rate.toFixed(1)}%</span>
                    </div>
              ...
              {!departmentPlacementRates.length && (
                <p className="text-xs text-muted-foreground">No department placement analytics available.</p>
              )}
            </CardContent>
          </Card>
        </div>
        ...
        <div className="lg:col-span-4 space-y-6">
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-border">
                {needsAction.map((action, i) => (
                  <div key={i} className="p-4 px-6 flex items-center justify-between hover:bg-muted/50 cursor-pointer group" onClick={action.onClick}>
                    <div className="flex items-center gap-3">
                ...
                {!needsAction.length && (
                  <p className="p-6 text-xs text-muted-foreground">No urgent actions from current API signals.</p>
                )}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-card border-border shadow-sm rounded-xl">
            <CardHeader className="p-6 pb-4 flex flex-row items-center justify-between">
              <CardTitle className="text-sm font-semibold text-foreground">Recent placements</CardTitle>
              <Button variant="outline" size="sm" className="h-8 bg-transparent border-border text-xs" onClick={() => navigate('/admin/applications')}>
                View applications →
              </Button>
            </CardHeader>
            <CardContent className="p-6 pt-0">
              <div className="space-y-4">
                {placements.map((placement) => (
                  <div key={placement._id} className="flex items-start gap-3">
                    <Avatar className="h-8 w-8 mt-0.5">
                      ...
                      <AvatarFallback>{placement.studentName?.slice(0, 2)?.toUpperCase() || 'ST'}</AvatarFallback>
                    </Avatar>
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-muted-foreground mt-0.5">
                        {placement.role} • {placement.package} • {formatRelativeTime(placement.hiredAt)}
                      </p>
                    </div>
                ...
                {!placements.length && (
                  <p className="text-xs text-muted-foreground">No recent placements from API.</p>
                )}
              </div>
            </CardContent>
          </Card>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-muted/50 border border-border p-4 rounded-xl">
                  ...
                  <p className="text-lg font-bold text-foreground mb-1">
                    {toNumber(packageComparison?.averagePackage?.currentYear, null) !== null
                ...
                <div className="bg-muted/50 border border-border p-4 rounded-xl">
                  ...
                  <p className="text-lg font-bold text-foreground mb-1">
                    {toNumber(packageComparison?.highestPackage?.currentYear, null) !== null
                ...
                <div className="bg-muted/50 border border-border p-4 rounded-xl">
                  ...
                  <p className="text-lg font-bold text-foreground mb-1">{computed.companiesVisited}</p>
                  <p className="text-[10px] text-muted-foreground">{openJobs.length} still active</p>
                </div>
                <div className="bg-muted/50 border border-border p-4 rounded-xl">
                  <p className="text-[10px] text-muted-foreground font-medium uppercase mb-1">Offer accept rate</p>
                  <p className="text-lg font-bold text-foreground mb-1">{computed.offerAcceptRate}%</p>
                  <p className="text-[10px] text-muted-foreground">{computed.pendingOffers} pending</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-card border border-border p-5 rounded-xl hover:bg-muted transition-colors cursor-pointer group flex flex-col items-center justify-center text-center gap-2" onClick={() => navigate('/admin/applications')}>
                ...
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground mt-0.5">{computed.pendingReviewCount} waiting</p>
                </div>
              </div>
              <div className="bg-card border border-border p-5 rounded-xl hover:bg-muted transition-colors cursor-pointer group flex flex-col items-center justify-center text-center gap-2" onClick={() => navigate('/admin/students')}>
                ...
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground mt-0.5">{toNumber(summary.students?.value, 0)} registered</p>
                </div>
              </div>
              <div className="bg-card border border-border p-5 rounded-xl hover:bg-muted transition-colors cursor-pointer group flex flex-col items-center justify-center text-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground mt-0.5">CSV · semester data</p>
                </div>
              </div>
            </div>
          </div>
