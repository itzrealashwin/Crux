...
// ─── Normalize helpers ────────────────────────────────────────────────────────
// These make filter matching resilient to API casing differences
const normalizeJobType = (raw = "") => {
  const v = raw.toLowerCase().replace(/\s+/g, " ").trim();
  if (v.includes("intern") && v.includes("ppo")) return "Intern + PPO";
  if (v.includes("intern"))                        return "Internship";
  return "Full time";                               // default
};

const normalizeWorkMode = (raw = "") => {
...
const JobListingPage = () => {
  ...
  const [filters, setFilters] = useState({
    eligibility: [],   // "Eligible for me" | "Borderline" | "All drives"
    jobType:     [],   // "Full time" | "Internship" | "Intern + PPO"
    workMode:    [],   // "On-site" | "Hybrid" | "Remote"
    status:      ["Open"], // "Open" | "Closing soon" | "Already applied"
    minPackage:  0,
  });
  ...
  const clearAllFilters = () => {
    setFilters({ eligibility: [], jobType: [], workMode: [], status: [], minPackage: 0 });
    setSearchQuery("");
  };
  ...
  const getEligibilityStatus = (job) => {
    ...
    if (!deptEligible || !cgpaEligible || !backlogEligible) {
      return {
        status: "Not eligible",
        variant: "destructive",
        reason: !cgpaEligible ? `${criteria.minCgpa} CGPA needed` : "Criteria unmet",
      };
    }
    ...
    if (missingSkills.length > 0) return { status: "Skills gap", variant: "secondary" };

    return { status: "Fully eligible", variant: "default" };
  };
  ...
  const filteredAndSortedJobs = useMemo(() => {
    let result = jobs.filter(job => {
      ...
      // 3. Job Type  (normalize before comparing)
      if (filters.jobType.length > 0 && !filters.jobType.includes(normalizeJobType(job.type))) return false;

      // 4. Work Mode (normalize before comparing)
      if (filters.workMode.length > 0 && !filters.workMode.includes(normalizeWorkMode(job.workMode))) return false;
      ...
      if (filters.status.length > 0) {
        ...
        const deadlineDays  = job.deadline
          ...
          : Infinity;
        const isClosingSoon = isOpen && deadlineDays >= 0 && deadlineDays <= 3;

        // Build the set of status labels this job matches
        const jobStatusLabels = new Set();
        if (isOpen)        jobStatusLabels.add("Open");
        if (isClosingSoon) jobStatusLabels.add("Closing soon");
        // "Already applied" — requires knowing the student applied; skip server-side for now
        // but honour the filter if the job carries an `applied` flag
        if (job.applied)   jobStatusLabels.add("Already applied");

        const matches = filters.status.some(s => jobStatusLabels.has(s));
        if (!matches) return false;
      }

      // 6. Eligibility filter
      if (filters.eligibility.length > 0 && !filters.eligibility.includes("All drives")) {
        const elig = getEligibilityStatus(job);
        const wantsEligible   = filters.eligibility.includes("Eligible for me");
        const wantsBorderline = filters.eligibility.includes("Borderline");

        const isEligible   = elig.status === "Fully eligible";
        const isBorderline = elig.status === "Skills gap";

        if (wantsEligible   && !wantsBorderline && !isEligible)   return false;
        if (wantsBorderline && !wantsEligible   && !isBorderline) return false;
        if (wantsEligible   && wantsBorderline  && !isEligible && !isBorderline) return false;
      }
    ...
    if (sortBy === "deadline_soonest") {
      ...
      // Assumes jobs have a createdAt field; falls back to array order if not
      result.sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0));
    }

    return result;
  }, [jobs, filters, searchQuery, sortBy, studentProfile]);
  ...
  return (
    <div className="pb-12 max-w-[1500px] mx-auto font-sans px-4 sm:px-6 md:px-8">
      <div className="flex flex-col md:flex-row gap-8 items-start mt-6">
        
        {/* --- SIDEBAR FILTERS --- */}
        <aside className="w-full md:w-72 shrink-0 hidden md:block space-y-8">
          <div className="flex items-center justify-between pb-2 border-b border-border/40">
            <h2 className="font-semibold text-lg">Filters</h2>
            <Button variant="ghost" size="sm" onClick={clearAllFilters} className="h-8 text-xs font-normal text-muted-foreground hover:text-foreground">
              Clear all
            </Button>
          </div>
          ...
          <div className="space-y-3">
            ...
            {["Eligible for me", "Borderline", "All drives"].map((label) => (
              <div key={label} className="flex items-center space-x-3">
                ...
                />
                <label htmlFor={`eligibility-${label}`} className="text-sm font-medium leading-none cursor-pointer flex-1">{label}</label>
              </div>
            ))}
          </div>
        ...
        <main className="flex-1 w-full space-y-6">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <Select value={sortBy} onValueChange={setSortBy}>
              <SelectTrigger className="w-full md:w-[220px] bg-muted/30 border-0">
              ...
              filteredAndSortedJobs.map((job, index) => {
                ...
                return (
                  <Card
                    ...
                    className="bg-card hover:bg-accent/10 transition-colors overflow-hidden border-0 shadow-sm cursor-pointer border-b md:border-b-0 md:rounded-xl rounded-none"
                  >
                    <CardContent className="p-6">
                      <div className="flex flex-col md:flex-row justify-between gap-6">
                        ...
                        <div className="flex gap-4">
                          <Avatar className="h-12 w-12 rounded-lg border-0 bg-muted/50">
                          ...
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {index === 0 && (
                                <Badge variant="secondary" className="bg-green-500/10 text-green-500 hover:bg-green-500/20 px-1.5 py-0 text-[10px] h-4">New</Badge>
                              )}
                            </div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-5">
                        <Badge variant="outline" className="rounded-full font-normal text-xs text-muted-foreground border-border/60">
                          {normalizeJobType(job.type)}
                        </Badge>
                        <Badge variant="outline" className="rounded-full font-normal text-xs text-muted-foreground border-border/60">
                      ...
                      {/* Bottom Actions */}
                      <div className="flex flex-wrap items-center justify-between mt-6 pt-5 gap-4">
                        ...
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            if (elig.status !== "Not eligible") apply(job._id || job.id);
                          }}
                        >
                          {isApplying && applyingId === (job._id || job.id)
                            ? <Loader2 className="h-4 w-4 animate-spin" />
                            : elig.status === "Not eligible" ? "Not eligible"
                            : elig.status === "Skills gap"   ? "Apply anyway"
                            : "Apply now"
                          }
                        </Button>
                      </div>
